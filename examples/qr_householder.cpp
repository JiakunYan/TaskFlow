...
int main()
{
  ...
  houseHolder.setTask([](int k) {
    ...
    // compute minor
    z1_.compute_minor(z_, k);
 
    // extract k-th column into x
    z1_.extract_column(x, k);
    ...
    // qv[k] = I - 2 *e*e^T
    compute_householder_factor(qv_[k], e);
